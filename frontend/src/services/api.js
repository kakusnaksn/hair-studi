// frontend/src/services/api.js
import axios from 'axios';

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001/api';
const apiClient = axios.create({
    baseURL: API_URL,
    headers: { 'Content-Type': 'application/json' },
});
apiClient.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('token');
    if (token) config.headers['x-auth-token'] = token;
    return config;
  },
  (error) => Promise.reject(error)
);

// Auth calls
export const registerUser = (userData) => apiClient.post('/auth/register', userData);
export const loginUser = (credentials) => apiClient.post('/auth/login', credentials);
export const requestPasswordReset = (email) => apiClient.post('/auth/request-password-reset', { email });
export const resetPassword = (token, newPassword) => apiClient.post('/auth/reset-password', { token, newPassword });


// Service calls
export const getAllServices = () => apiClient.get('/services');

// Schedule/Availability calls
export const getAvailableSlots = (serviceId, date, stylistId = null) => {
  let queryString = `serviceId=${serviceId}&date=${date}`;
  if (stylistId) queryString += `&stylistId=${stylistId}`;
  return apiClient.get(`/schedules/availability?${queryString}`);
};
export const getStylistMySchedule = (startDate, endDate) => { // Added params from actual use
    let queryString = '';
    if (startDate && endDate) {
        queryString = `?startDate=${startDate}&endDate=${endDate}`;
    }
    return apiClient.get(`/schedules/stylist/me${queryString}`);
};

// Appointment calls
export const createAppointment = (bookingData) => apiClient.post('/appointments', bookingData);
export const getMyAppointments = () => apiClient.get('/appointments/my-appointments');
export const updateAppointmentStatus = (appointmentId, status) => apiClient.patch(`/appointments/${appointmentId}/status`, { status });

// Admin specific calls
export const createStylistByAdmin = (stylistData) => apiClient.post('/admin/stylists', stylistData);

// User profile (example, if/when needed)
// export const getUserProfile = () => apiClient.get('/users/me');
